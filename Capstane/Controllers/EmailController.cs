		...
		public async Task<IActionResult> SendReport([FromBody] ReportRequest reportRequest)
		{
			try
			{
				...
				using (var csv = new CsvWriter(writer, CultureInfo.InvariantCulture))
				{
					...
					{
						csv.WriteField(ticket.Id);
						csv.WriteField(ticket.Subject ?? "N/A");
						csv.WriteField(ticket.Priority ?? "N/A");
						csv.WriteField(ticket.Status);
						csv.WriteField(ticket.AssignedAgent ?? "Unassigned");
						csv.WriteField(ticket.CreatedAt.ToString("u")); // Use universal time format
						csv.WriteField(ticket.UpdatedAt.ToString("u"));
						csv.NextRecord();
					}
				}
